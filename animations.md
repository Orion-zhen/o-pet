...
| 组合与抢占 | `renderer/host.js` | 接收外部事件并选择下一状态或行为 |
| 状态生命周期 | `renderer/runtime/host-state.js` | 集中停止旧导演、管理切换计时器和销毁 |
...
高层动画只使用一个有限时间线。新的 `timeline.play()` 会取消当前时间线，因此活动、空闲片段、Cue 和普通交互不会并行播放。角色的呼吸、眨眼、视线、弹簧和粒子仍由每帧运行时持续推进。运行时每帧提交新的只读快照，SVG 视图不会读取或修改弹簧与视觉通道的内部状态。`presenter` 可在受保护动画期间临时显示 `dragging`，但不会启动另一条有限时间线。覆盖期间，原时间线仍可进入后续基础场景，松开指针后显示当时最新的基础场景。
...
每次状态转换先按旧状态停止对应导演或时间线，再记录新状态。`switching` 的计时器也由状态生命周期模块持有，因此状态离开 `switching` 时必定取消尚未到期的切换。销毁会通过同一模块停止所有导演和时间线。
...
进入 `sleeping` 后持续 5–8 min，再自然醒来。自然醒播放随机方向的 `stretching` 3500 ms，随后通常播放 `happy` 1400 ms；15% 概率改为 `quickHappy` 900 ms 并眨眼。动画开始时即重置空闲会话，结束后从 `awake` 重新进入完整深度循环。同类自然醒的伸展方向尽量与上一次相反。自然醒不使用受保护的 `waking` 动画，也不改变 Agent 活动触发 `waking` 的现有规则。

基础场景的等待不会跨过深度边界或自然醒边界。到达边界时，导演立即重新计算深度并进入对应基础场景。页面隐藏的时间不计入空闲会话。
...
每个片段结束后回到当前深度的基础场景，再等待下一次片段。进入 Agent 活动会终止空闲会话。再次进入 `idle` 或自然醒时，深度、片段冷却、历史和能量全部重置。
...
系统 `prefers-reduced-motion` 启用时进入减少动态模式。高层场景顺序、步骤时长、惊醒顺序和空闲深度不变。帧运行时会压制旋转、跳跃、扑动等一次性位移，将主要身体变换收敛到静态值，并立即完成部分视觉通道过渡。

### 销毁

销毁顺序会停止切换计时器、指针监听、偏好监听、Cue、空闲、活动、交互、时间线、角色动画帧和统一调度器。销毁后的 `update()` 返回 `false`。`showAction()` 和偏好更新也不再启动动画。
