...
function create(math, tables) {
  ...
  /** @param {string} state @param {number} globalSec @param {number} localSec @param {number} _now @param {import("../../types.js").ExpressionContext} context @param {import("../../types.js").ControllerOptions} [options] */
    ...
    switch (state) {
      case "sleeping":
        if (EYE_PLAYLIST.sleeping?.includes(options.eyeTo ?? -1)) {
          restLid = (options.eyeMorphX ?? 1) > 0.85 ? 1 : 0.08;
          ...
          restLid = 0.08;
          if ((options.blinkX ?? 1) < 0.18) eyeTarget = [13, 11];
        }
        break;
      case "dreaming":
        restLid = 0.06;
