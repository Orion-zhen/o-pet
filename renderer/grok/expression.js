...
(function (g) {
  ...
  function sample(state, globalSec, localSec, _now, context, options = {}) {
    ...
    switch (state) {
      case "sleeping":
        if (EYE_PLAYLIST.sleeping.includes(options.eyeTo)) {
          ...
          restLid = Math.max(0.08, 1 - close * (1 + 0.15 * Math.sin(localSec * 6.5)));
        } else {
          restLid = 0.08;
      ...
      case "drowsy":
        ...
        if (context.slumpAt) {
          const elapsed = (_now - context.slumpAt) / 1000;
      ...
      case "celebrate":
        ...
        break;
      case "dragging":
        if ((localSec % 3.4) / 3.4 >= 0.12 && (localSec % 3.4) / 3.4 < 0.62) eyeScale = 1.06;
        break;
      case "notifying":
        if (context.notifyBlink) {
          context.notifyBlink = false;
          requestBlink = true;
        }
    ...
    return { restLid, eyeScale, faceRollDeg, eyeLids, eyeTarget, requestBlink };
  }

  g.GROK_EXPRESSION = Object.freeze({ sample });
})(window);
