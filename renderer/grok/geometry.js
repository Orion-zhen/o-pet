...
(function (g) {
  ...
  const lerpPoly = (from, to, amount) => from.map((point, index) => [
  ...
  const lerpFace = (from, to, amount) => ({
    ...
    leftDX: (from.leftDX ?? 0) + ((to.leftDX ?? 0) - (from.leftDX ?? 0)) * amount,
  });
    ...
    while (index < tokens.length) {
      ...
      if (command === "Z") {
        if (Math.hypot(startX - x, startY - y) > 0.01) {
          sample(
            (amount) => [x + (startX - x) * amount, y + (startY - y) * amount],
      ...
      if (command === "M") {
        ...
        sample((amount) => {
          ...
          return [
            inverse * inverse * fromX + 2 * inverse * amount * controlX + amount * amount * nextX,
            inverse * inverse * fromY + 2 * inverse * amount * controlY + amount * amount * nextY,
          ];
        }, Math.hypot(controlX - x, controlY - y) + Math.hypot(nextX - controlX, nextY - controlY));
        x = nextX;
        y = nextY;
      } else if (command === "C") {
  ...
  function flattenRingPath(path, step = 4) {
    ...
    const sample = (curve, length) => {
      const count = Math.max(2, Math.ceil(length / step));
      for (let sampleIndex = 1; sampleIndex <= count; sampleIndex++) points.push(curve(sampleIndex / count));
    };
    while (index < tokens.length) {
      if (/[a-z]/i.test(tokens[index])) command = tokens[index++].toUpperCase();
      if (command === "Z") {
  ...
  function spanPoly(points, y, center) {
    ...
    return [Number.isFinite(left) ? left : center, Number.isFinite(right) ? right : center];
  }
  ...
  function polarRing(points, center, count = DEFAULT_RING_POINTS) {
    return Array.from({ length: count }, (_, index) => {
      ...
      for (let pointIndex = 0; pointIndex < points.length; pointIndex++) {
        ...
        const intersection = (fromX + (toX - fromX) * amount) * rayX
  ...
  function solidRadii(solid, angle, count = DEFAULT_RING_POINTS) {
    const cosine = Math.cos(angle);
    const sine = Math.sin(angle);
    const rotated = solid.map(([x, y, z, radius]) => [x * cosine + z * sine, y, radius]);
    const raw = Array.from({ length: count }, (_, index) => {
      ...
      for (const [x, y, sphereRadius] of rotated) {
        ...
        const discriminant = projection * projection - (x * x + y * y) + sphereRadius * sphereRadius;
  ...
  function makeTurnAt(solid, ring, center) {
    ...
    return (yaw) => {
      ...
      for (let pass = 0; pass < 3; pass++) {
        ...
        scale = previous.map((value, index) => (
  ...
  function shapeModel(name) {
    ...
    const data = g.GROK_GEO.shapes[name];
    if (!data) return null;
    const center = g.GROK_GEO.Re;
    ...
    const solid = g.GROK_GEO.solids?.[name];
    model = Object.freeze({
      name,
      data,
      flattened,
  ...
  function formRing(kind, center, teardropPath) {
    if (kind === "pencil" && teardropPath) {
      return rotateRing(polarRing(flattenRingPath(teardropPath), center), DEFAULT_RING_POINTS / 2, center);
    }
    return circleRing(center);
  }

  g.GROK_GEOMETRY = Object.freeze({
    DEFAULT_RING_POINTS,
    DEFAULT_SPAN_ROWS,
    polyPath,
    centroid,
