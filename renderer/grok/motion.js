...
(function (g) {
    ...
    switch (state) {
      ...
      case "dreaming": {
        ...
        if (!options.reduceMotion) {
          ...
          } else if (variant === "twitch" && localSec > 3.2 && localSec < 3.75) {
      ...
      case "waking":
        if (localSec < 0.5) {
          ...
          rollDeg = restRollDeg + Math.sin(settle * Math.PI * 3) * 6 * (1 - settle);
      ...
      case "idle": {
        ...
        const breathing = Math.sin(globalSec * 0.78 + Math.sin(globalSec * 0.09) * 0.45);
        let shift = 0;
        if (now < context.idleShiftEnd) {
          const elapsed = 1 - (context.idleShiftEnd - now) / context.idleShiftDuration;
          shift = Math.sin(clamp(elapsed, 0, 1) * Math.PI) * context.idleShiftDirection;
        }
        rollDeg = restRollDeg + Math.sin(globalSec * 0.31) * 0.8 + shift * 1.8;
        xPx = Math.sin(globalSec * 0.19) * 0.7 + shift * 2.2;
        yPx = breathing * 1.1;
        squashY = 1 + breathing * (0.006 + 0.002 * Math.sin(globalSec * 0.07));
        break;
      }
      case "listening":
      ...
      case "angry":
        ...
        rollDeg = restRollDeg + (now < context.angryShakeUntil ? Math.sin(now * 0.05) * 4.5 : 0);
        yPx = 3.5;
        squashY = 0.975;
        break;
      case "startled": {
      ...
      case "stretching": {
        ...
        if (localSec < 0.45) {
          ...
          yPx = -4;
          rollDeg = direction * (8 + Math.sin(amount * Math.PI * 6) * (options.reduceMotion ? 0 : 0.7));
          squashY = 1.105;
        } else {
      ...
      case "quizzical": {
        ...
        else if (localSec < 1.45) amount = 1 + 0.2 * K2((localSec - 0.55) / 0.9);
      ...
      case "drowsy": {
        ...
        if (now >= context.nodUntil && !context.slumpAt) context.slumpAt = now;
        if (context.slumpAt) {
          ...
          if (elapsed < sinkDuration) {
            ...
            const wobble = Math.sin(phase * Math.PI * 2.5) * 2.2 * (1 - phase);
            ...
            const phase = (elapsed - sinkDuration - wakeDuration) / recoverDuration;
  ...
  g.GROK_MOTION = Object.freeze({ sample });
})(window);
