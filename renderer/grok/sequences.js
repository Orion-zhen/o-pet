...
(function (g) {
  const S = g.GROK_PRESETS.scenes;
  const step = (scene, duration, options = {}) => Object.freeze({ scene, duration, ...options });
  ...
  const cues = Object.freeze({
    ...
    error_repeated: sequence(step(S.confused, 1200, { preserveEffect: true })),
    ...
    completed_hard: sequence(step(S.celebrate, 2500), step(S.notifying, 2500)),
    run_failed: sequence(step(S.sad, 1800), step(S.notifying, 1600)),
    run_aborted: sequence(step(S.surprised, 600)),
  });

  const fullWake = (direction) => sequence(
    step(g.GROK_PRESETS.withDetails(S.stretching, { direction }), 3500),
    step(S.playful, 700),
    step(S.happy, 900),
  );

  g.GROK_SEQUENCES = Object.freeze({ cues, fullWake, step, sequence });
})(window);
